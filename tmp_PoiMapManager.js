...
// Do not globally silence `console.debug` — use `window.POI_DEBUG` to control debug output.
...
export default class PoiMapManager {
    constructor(options) {
        this.options = Object.assign({
            ...
            poiAggGrid: 10000,
            // When true, update the browser URL query params on map moveend
            updateUrl: false
        }, options || {});
    ...
    initMap() {
        ...
        this.map.on('moveend', () => {
                ...
                this.debounceTimeout = setTimeout(() => {
                    // Optionally update URL with current center/zoom (can be disabled)
                    try {
                        if (this.options.updateUrl && window && window.history && window.location) {
    ...
    updateSelectedUI() {
        ...
        // NOTE: delegated import-button listener registered once in `bindUIEvents()`
    ...
    async fetchAndPlot(opts = {}) {
        ...
        // Request-id counter (simple increment for diagnostics).
        // Capture a local `reqId` so we can detect stale/out-of-order fetches
        // and avoid applying results from older requests.
        let __reqId = null;
        try {
            if (!this._fetchRequestCounter) this._fetchRequestCounter = 0;
            this._fetchRequestCounter = Number(this._fetchRequestCounter) + 1;
            __reqId = this._fetchRequestCounter;
            this._currentFetchRequestId = __reqId;
            if (window.POI_DEBUG && console && console.debug) console.debug('fetchAndPlot requestId', __reqId);
        } catch (e) {}
        ...
        // Normalize query components to create a stable dedupe key
        const normalizedSearch = (search || '').toString().trim().toLowerCase();
        const normalizedTypes = Array.isArray(types) ? types.slice().filter(Boolean).map(t => String(t)).sort() : [];
        const bboxStr = bounds.toBBoxString();
        const queryKeyObj = { bbox: bboxStr, zoom: Number(zoom), search: normalizedSearch, types: normalizedTypes, onlyMine: !!onlyMine };
        const queryKey = JSON.stringify(queryKeyObj);

        // DEBUG: log queryKey vs lastQueryKey to diagnose repeated-search issues
        try { console.log('POI: queryKey:', queryKey, 'lastQueryKey:', this.lastQueryKey, 'opts.force:', !!(opts && opts.force)); } catch (e) {}
        if (queryKey === this.lastQueryKey && !opts.force) {
            try { console.log('POI: fetchAndPlot skipped due to identical queryKey and no force flag'); } catch (e) {}
            return;
        }
        ...
        try {
            ...
            if (onlyMine) {
                ...
                if (!response) {
                    try {
                        ...
                        const bboxParam = [bounds.getSouth(), bounds.getWest(), bounds.getNorth(), bounds.getEast()].join(',');
                        const params2 = new URLSearchParams({ bbox: bboxParam, limit: 2000, mine: onlyMine ? 1 : 0, types: (serverTypes && serverTypes.length) ? serverTypes.join(',') : '', search: search || '' });
                ...
                // If both server-side Overpass and MySQL returned no rows for a
                // non-empty search term, prefer asking the server proxy again
                // (do NOT post directly from the browser to public Overpass mirrors).
                if ((!data || data.length === 0) && search && String(search).trim() !== '' && !onlyMine) {
                    try {
                        if (window.POI_DEBUG && console && console.debug) console.debug('Attempting server-proxy Overpass fallback for search', search);
                        // reuse the prepared query string used for the server overpass call
                        const proxyCandidates = PoiMapManager.makeApiCandidates('api/locations/search_overpass_v2.php', queryStr);
                        let proxyResp = null;
                        for (const pc of proxyCandidates) {
                            try {
                                if (window.POI_DEBUG && console && console.debug) console.debug('Proxy candidate ->', pc);
                                const resp = await fetch(pc, { credentials: 'same-origin', signal: AbortSignal.timeout(this.options.overpassTimeoutMs) });
                                if (resp && resp.ok) { proxyResp = resp; if (window.POI_DEBUG && console && console.debug) console.debug('Proxy success', pc); break; }
                            } catch (e) {
                                if (window.POI_DEBUG && console && console.debug) console.debug('Proxy candidate failed', pc, e && (e.message || e));
                                continue;
                            }
                        }
                        if (proxyResp) {
                            try {
                                ...
                                const rowsPub = Array.isArray(j.data) ? j.data : (Array.isArray(j) ? j : []);
                                if (rowsPub && rowsPub.length) {
                                    data = rowsPub;
                                    if (window.POI_DEBUG && console && console.debug) console.debug('Server-proxy Overpass returned rows', rowsPub.length);
                                }
                            } catch (e) {
                                if (window.POI_DEBUG && console && console.debug) console.debug('Failed to parse proxy Overpass response', e);
                            }
                        }
                    } catch (e) {
                        if (window.POI_DEBUG && console && console.debug) console.debug('Server-proxy Overpass fallback failed', e);
                    }
                }
            ...
            try {
                if (window.CURRENT_USER_ID) {
                    ...
                    const paramsUser = new URLSearchParams({ bbox: bboxParamUser, limit: 2000, mine: 1, types: (serverTypes && serverTypes.length) ? serverTypes.join(',') : '', search: search || '' });
            ...
            // If another fetch has started since this one began, treat this
            // response as stale and do not modify the map state.
            try {
                if (typeof __reqId !== 'undefined' && this._fetchRequestCounter !== __reqId) {
                    if (window.POI_DEBUG && console && console.debug) console.debug('fetchAndPlot: stale response, abort', { reqId: __reqId, current: this._fetchRequestCounter });
                    return;
                }
            } catch (e) {}
            ...
            try {
                ...
                if (shouldCenter && data.length > 0 && this.map) {
                    // If caller provided explicit geocode coords, prefer centering there
                    if (opts && opts.center && Number.isFinite(opts.centerLat) && Number.isFinite(opts.centerLon)) {
                        try {
                            const lat = Number(opts.centerLat);
                            const lon = Number(opts.centerLon);
                            const currentZoom = this.map.getZoom();
                            const targetZoom = currentZoom < 14 ? 15 : currentZoom;
                            if (window.POI_DEBUG && console && console.debug) console.debug('Centering map to explicit center coords', { lat, lon, currentZoom, targetZoom });
                            this.map.setView([lat, lon], targetZoom, { animate: true });
                            // Try to find a marker at these coords and open its popup
                            try {
                                const tol = 1e-5;
                                let m = null;
                                for (const k in this.markerByOsm) {
                                    try {
                                        ...
                                        if (Math.abs(ll.lat - lat) <= tol && Math.abs(ll.lng - lon) <= tol) { m = cand; break; }
                                    } catch (e) { continue; }
                                }
                                if (m && typeof m.openPopup === 'function') {
                                    this.setMarkerSelected(m, true);
                                    const showAndOpen = () => { try { if (this.markers && typeof this.markers.zoomToShowLayer === 'function') { this.markers.zoomToShowLayer(m, () => { try { m.openPopup(); } catch(e){} }); return; } m.openPopup(); } catch(e){} };
                                    try { if (this.map && typeof this.map.once === 'function') { this.map.once('moveend', showAndOpen); setTimeout(showAndOpen,700); } else setTimeout(showAndOpen,300); } catch(e){ setTimeout(showAndOpen,300); }
                                }
                            } catch (e) {}
                        } catch (e) {}
                        // explicit center handled — skip the search-based auto-center below
                    } else {
                        ...
                        try {
                            ...
                            if (sRaw.length) {
                                ...
                                if (toks.length) {
                                    // Score each POI giving strong preference to token matches in the POI name
                                    // This reduces accidental centering on unrelated results that match
                                    // tokens only in city/state/country fields.
                                    let best = null; let bestScore = 0;
                                    for (const p of data) {
                                        try {
                                            if (!p) continue;
                                            const nameHay = (p.name || '').toString().toLowerCase();
                                            const hay = [p.name, p.type, p.description, p.city, p.state, p.country].filter(Boolean).join(' ').toLowerCase();
                                            let scoreName = 0, scoreOther = 0;
                                            for (const t of toks) {
                                                if (!t) continue;
                                                if (nameHay.includes(t)) scoreName++;
                                                else if (hay.includes(t)) scoreOther++;
                                            }
                                            // Boost prefix matches in the name (likely better intent)
                                            for (const t of toks) {
                                                if (nameHay.startsWith(t)) scoreName += 2;
                                            }
                                            // Combine scores: name matches are weighted much higher
                                            const score = (scoreName * 10) + scoreOther;
            ...
            // Record successful query key so repeated identical requests are deduplicated.
            try {
                // Only record lastQueryKey for the active (non-stale) request
                if (typeof __reqId !== 'undefined' && this._fetchRequestCounter === __reqId) {
                    this.lastQueryKey = queryKey;
                } else {
                    if (window.POI_DEBUG && console && console.debug) console.debug('Not setting lastQueryKey due to stale/overridden request', { reqId: __reqId, current: this._fetchRequestCounter });
                }
            } catch (e) {}
    ...
    bindUIEvents() {
        ...
        const applyFilterHandler = async (ev) => {
            ev.preventDefault();
            ...
            // If a search term is present, prefer forcing an Overpass lookup
            // so users get OSM results even when the MySQL DB has no matching rows.
            try {
                const s = (document.getElementById('poi-search') && String(document.getElementById('poi-search').value || '').trim()) || '';
                // mark that user initiated a load so auto-centering logic can run
                this.hasLoadedOnce = true;
                if (s) {
                    // If the map is at a very low zoom (world view), first attempt a
                    // light-weight geocode to move the map to the searched place before
                    // querying Overpass. This avoids global bbox queries returning
                    // unrelated POIs (e.g. waste_disposal matches in distant cities).
                    try {
                        const currentZoom = this.map && this.map.getZoom ? this.map.getZoom() : 2;
                        const shouldGeocode = currentZoom < 8; // tune threshold as needed
                        if (shouldGeocode) {
                            // quick nominatim lookup (limit=1). If it succeeds, center map
                            // and then fetch; otherwise fall back to direct fetch.
                            const nomUrl = 'https://nominatim.openstreetmap.org/search?format=json&limit=1&q=' + encodeURIComponent(s);
                            try {
                                if (window.POI_DEBUG && console && console.debug) console.debug('Attempting geocode for search', s, nomUrl);
                                ...
                                if (r && r.ok) {
                                    ...
                                    if (Array.isArray(arr) && arr.length) {
                                        ...
                                        if (Number.isFinite(lat) && Number.isFinite(lon)) {
                                            // Request fetch and pass the geocode coords so fetchAndPlot
                                            // will center on the geocode location rather than selecting
                                            // a server-side 'firstPoi' which might be unrelated.
                                            try {
                                                this.fetchAndPlot({ force: true, forceOverpass: true, center: true, centerLat: lat, centerLon: lon });
                                                // also refresh full list after a short delay
                                                setTimeout(() => { try { this.fetchAndRenderFullPoiList(); } catch(e){} }, 700);
                                            } catch (e) {}
                                            return;
                                        }
                                    }
                                ...
                                if (window.POI_DEBUG && console && console.debug) console.debug('Geocode failed, falling back to direct fetch', e);
                            }
                        }
                    } catch (e) {}
                    // fallback: do direct forced fetch and request centering
                    this.fetchAndPlot({ force: true, forceOverpass: true, center: true });
                } else {
        ...
        // Defensive: ensure repeated searches always trigger a fetch.
        // Some browsers or runtime states may prevent earlier handlers from
        // executing reliably; add a resilient listener that always invokes
        // `fetchAndPlot` with `force:true` and logs to the console.
        try {
                if (searchBtn) {
                searchBtn.addEventListener('click', (ev) => {
                    try {
                        console.log('POI: resilient search-btn click, invoking fetchAndPlot');
                        ev.preventDefault();
                        this.hasLoadedOnce = true;
                        this.fetchAndPlot({ force: true, forceOverpass: true, center: true }).catch(e => { if (console && console.error) console.error('fetchAndPlot error', e); });
                    } catch (e) { if (console && console.error) console.error('resilient click handler failed', e); }
                });
            }
            if (searchInput) {
                searchInput.addEventListener('keydown', (ev) => {
                    if (ev.key === 'Enter') {
                        try {
                            console.log('POI: resilient search-input Enter, invoking fetchAndPlot');
                            ev.preventDefault();
                            this.hasLoadedOnce = true;
                            this.fetchAndPlot({ force: true, forceOverpass: true, center: true }).catch(e => { if (console && console.error) console.error('fetchAndPlot error', e); });
                        } catch (e) { if (console && console.error) console.error('resilient key handler failed', e); }
                    }
                });
            }
        } catch (e) {}
        ...
        // One-time delegated listener for import buttons (fallback)
        try {
            if (!this._delegatedImportListenerBound) {
                this._delegatedImportListener = (ev) => {
                    try {
                        const tgt = ev.target && ev.target.closest ? ev.target.closest('.poi-import-btn') : null;
                        if (!tgt) return;
                        try { if (window.POI_DEBUG && console && console.debug) console.debug('delegated import click', { href: tgt.href || null, osmId: tgt.dataset && tgt.dataset.osmId, osmType: tgt.dataset && tgt.dataset.osmType, pvBound: tgt.dataset && tgt.dataset.pvBound }); } catch (e) {}
                        if ((!tgt.dataset || !tgt.dataset.pvBound) && typeof this.doImportPoi === 'function') {
                            try { ev.preventDefault(); ev.stopPropagation(); } catch (e) {}
                            try { this.doImportPoi(tgt.dataset.osmType, tgt.dataset.osmId); } catch (e) { console.error('Fallback import call failed', e); }
                        }
                    } catch (e) {}
                };
                document.addEventListener('click', this._delegatedImportListener, true);
                this._delegatedImportListenerBound = true;
            }
        } catch (e) {}
    }
